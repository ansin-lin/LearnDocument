...
Java 8 后使用 Metaspace，存储类元信息、常量、方法信息等。

Metaspace 使用本地内存，不再固定在 JVM 堆内。

---

#### 1.3 栈

每个线程都有自己的虚拟机栈。方法调用会创建栈帧，栈帧中保存：
...
记录当前线程执行到哪一条字节码指令。线程私有，占用很小。

---

### 2. GC 垃圾回收

#### 2.1 CMS / G1 / ZGC / Shenandoah

| GC | 特点 |
| --- | --- |
| CMS | 低停顿，老年代收集器，已逐渐淘汰 |
| G1 | 分区回收，兼顾吞吐和停顿，常用 |
| ZGC | 超低延迟，适合大内存低停顿场景 |
| Shenandoah | 低停顿 GC，和 ZGC 类似方向 |
...
从 GC Roots 可达的对象不会被回收。

---

#### 2.3 STW

Stop The World，指 GC 时暂停用户线程。

STW 时间过长会导致系统响应变慢，甚至请求超时。

---

#### 2.4 对象晋升机制

对象一般先分配在 Eden 区。经过多次 Minor GC 后仍然存活，会晋升到 Old 区。
...
Memory Analyzer Tool，用于分析 heap dump。
