...
> **事故处理章节：** 请在掌握基本提交、分支和远程同步后学习。第一次操作 `reset`、`revert` 或 `reflog` 时使用独立练习仓库；工作项目发生事故时先保留现场并按团队流程报告。
...
| 当前情况 | 常用操作 | 是否改写历史 |
|---|---|---|
| 未暂存修改不需要了 | `git restore FILE_PATH` | 否，但未提交内容会丢失 |
| 已暂存，想取消暂存 | `git restore --staged FILE_PATH` | 否 |
| 最近提交信息或漏文件，尚未共享 | `git commit --amend` | 是 |
| 本地提交要拆回暂存区 | `git reset --soft HEAD~1` | 是 |
| 公共分支已有错误提交 | `git revert COMMIT_ID` | 否，新增反向提交 |
