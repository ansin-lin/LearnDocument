...
对象属性、类型别名、可选属性、只读属性、嵌套对象和对象数组属于【必须掌握】；复杂的结构兼容与多余属性检查先达到【理解即可】。
...
类型别名不只可以命名对象。第五章会用它命名联合类型，第六章会用它描述函数类型。
...
条件已经确认 `nickname` 不是 `undefined` 后，才能安全调用 `toUpperCase()`。第七章会系统讲解这种类型收窄。
...
断言后编译器相信 `name` 是字符串，但运行时真实值仍然是数字。第七章会学习检查未知值，第十四章会把检查应用到接口响应。
