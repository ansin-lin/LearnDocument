...
- 【必须掌握】基础泛型函数、类型参数推断、数组类型、`Promise<T>`以及分页结构`Page<T>`。
- 【项目代码阅读】多个类型参数、泛型回调和泛型接口；【进阶】泛型约束。先能看懂类型关系，不要求新人立即设计复杂抽象。
...
泛型不会让函数内部获得“所有类型的所有方法”。它反而要求实现只使用当前类型参数明确保证的能力。第八节会通过泛型约束增加这种保证。
...
结果会保持元素类型：数字数组得到 `number | undefined`，字符串数组得到 `string | undefined`。使用结果前按第七章处理空值。
...
- 能使用泛型类型别名或接口表示`Page<T>`一类通用结构。
- 能读懂多个类型参数、泛型接口和`extends`约束。
