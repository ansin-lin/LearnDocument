...
第七章会使用 `typeof` 等判断安全地缩小 `unknown` 的范围。
...
【理解即可】本节只认识 `as` 的作用和风险。具体使用条件、非空断言以及安全的替代方法放到[第 7 章](07_narrowing_assertions.md)统一讲解。
...
本例中的 `as number` 在编译后会消失，真实值仍是字符串。需要数字时应使用前面学过的 `Number()`；需要确认外部数据时，应在第 7 章学习真实检查。
...
2. 删除断言，改用 `Number()` 和 `Number.isFinite()` 得到数字 `13`。
