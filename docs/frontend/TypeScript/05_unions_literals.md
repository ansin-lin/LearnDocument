...
`Math.random()` 返回一个大于等于 `0`、小于 `1` 的随机数，所以运行时 `code` 可能得到字符串，也可能得到数字。错误并不是说字符串不能转大写，而是当前代码位置还不能确定它一定是字符串。第七章会学习先判断类型，再使用对应方法。
...
第七章会学习通过条件判断缩小联合类型；第十四章会把运行时检查应用到请求数据。
