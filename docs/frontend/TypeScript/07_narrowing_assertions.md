...
`typeof`、空值判断、相等判断、判别属性、`Array.isArray()` 和对 `unknown` 的检查属于【必须掌握】。`in`、`instanceof`、自定义类型守卫与非空断言属于【项目代码阅读】，需要理解项目代码为什么这样写，不要求一开始独立设计复杂守卫。
