...
本章仍保留在一个文件中，但分四个阶段学习。第一次学习先完成前三个阶段，第四阶段在项目需要时查阅。

| 阶段 | 对应小节 | 学习成果 |
| --- | --- | --- |
| DOM基础与元素获取 | 第1～10节 | 找到单个或多个页面元素，并处理未找到的情况 |
| 内容、状态和属性 | 第11～15节 | 修改文字、表单值、属性、`dataset`和class |
| 常用节点操作 | 第16～19节 | 查找相邻元素，创建、插入和删除节点 |
| 扩展节点操作 | 第20～21节 | 看懂克隆和`DocumentFragment` |
| 综合操作与排错 | 第22～24节 | 按完整顺序完成并验证DOM任务 |
