...
## 3. 在 JavaScript 中转换 JSON

### 3.1 使用 `JSON.stringify()` 序列化
...
外部接口、手工编辑的数据或旧版存储内容可能不是合法 JSON。解析不可信内容时，应使用 `try...catch` 捕获异常，避免后续代码直接被打断。

`try`代码块放可能失败的操作；发生异常时，跳过try中尚未执行的代码，转去执行`catch`代码块。`catch (error)`中的error接收异常对象；没有异常时不执行catch。
...
`parseApplication()` 接收 JSON 字符串；成功时执行try中的return，失败时进入catch，记录错误后返回null。`console.error()`把错误信息输出到Console，不会自动修复或重试。

把调用参数换成 `'{"employeeName":}'` 再运行：应记录解析错误，后面的姓名输出不执行。恢复合法JSON后，应输出田中。这里用null作为此函数约定的失败标志；它不是通用的JSON合法性判断，因为JSON文本`"null"`本身也合法。

错误对象、主动抛错和 `finally` 可回顾[错误处理与浏览器调试](16_error_debugging.md)。本章 11.6 节进一步区分存储缺失、解析失败和数据结构错误。

现在已经能完成“对象 → JSON 文本 → 对象”的往返。可以分别输出转换前后的 `typeof`：对象是 `object`，JSON 文本是 `string`。接下来把中间的文本保存起来，再读取并还原；不要省略文本转换这一步。

## 4. 浏览器 Web Storage 的作用和边界
...
执行顺序是：读取 JSON 文本；key 不存在时使用空数组；存在时用 `JSON.parse()` 还原数组。如果内容可能被手工修改，还应参考 3.3 节处理解析失败，完整读取流程见 11.6 节。
...
单独准备以下文件，通过同一台本地服务器在**同一标签页**打开 `apply.html`。它们使用固定的演示草稿，只验证“保存 → 跳转 → 读取”，不依赖其他章节的项目页面。
...
<body>
  <p>演示草稿：有給休暇，开始日期 2026-09-01</p>
  <button id="nextButton" type="button">保存并查看</button>
  <p id="message"></p>
</body>
</html>
```
...
nextButton.addEventListener("click", () => {
  ...
  try {
    sessionStorage.setItem("lesson18.draft", JSON.stringify(draftApplication));
    location.href = "confirm.html";
  } catch (error) {
    console.error("草稿保存失败", error);
    message.textContent = "草稿未保存，请检查浏览器存储设置后重试";
  }
});
```

先保存成功，再修改 `location.href` 跳转。若保存失败，留在原页显示提示，不把用户带到没有草稿的确认页。
...
<body>
  <p id="draftText"></p>
  <p id="message"></p>
  <button id="clearButton" type="button">清除草稿并返回</button>
</body>
</html>
```
...
function showDraft() {
  try {
    const savedText = sessionStorage.getItem("lesson18.draft");
    if (savedText === null) {
    ...
    if (draft === null || typeof draft !== "object" || Array.isArray(draft)) {
      throw new Error("草稿必须是对象");
    }
    if (typeof draft.leaveType !== "string" || typeof draft.startDate !== "string") {
      throw new Error("草稿缺少有效的类型或日期字段");
    }

    draftText.textContent = draft.leaveType + "：" + draft.startDate;
  } catch (error) {
    console.error("草稿读取失败", error);
    message.textContent = "草稿无法读取，请检查数据；需要重新开始时可清除本次演示草稿";
  }
}

clearButton.addEventListener("click", () => {
  try {
    sessionStorage.removeItem("lesson18.draft");
    location.replace("apply.html");
  } catch (error) {
    console.error("草稿清除失败", error);
    message.textContent = "清除失败，当前草稿未确认清除，请重试";
  }
});
...
这里应用了 BOM 章节的跳转方法：普通流程用 `location.href`；无草稿或清理后返回用 `location.replace()`，避免返回键再次进入这次确认页。`return` 用于停止无草稿时的后续处理。

读取时先检查是否有值，再解析，最后检查本例需要的字段。解析失败时不自动覆盖旧数据；只有用户点击清除按钮，才删除 `lesson18.draft`。日期字段这里只检查字符串类型，不代替日期有效性校验。

验证：

1. 点击保存按钮，确认跳转后显示草稿，刷新确认页仍能看到。
2. 点击清除并返回，再直接访问确认页，应回到申请页。
3. 重新保存，在开发者工具中把该 key 的值改成 `not-json` 或 `{}`，刷新确认页，应显示错误而不是正常草稿。
4. 点击清除按钮恢复初始状态。仅清理本例 key，不清空其他数据。

### 6.5 URL 参数与临时存储如何选择

| 数据需求 | 使用方式 | 示例 |
| --- | --- | --- |
| 地址需要刷新、收藏或分享 | URL 参数 | 记录编号、页码、筛选条件 |
| 当前多页面流程临时使用完整数据 | `sessionStorage` | 尚未提交的表单草稿 |
| 需要后端长期可靠保存 | 后续学习的 HTTP 请求 | 正式申请记录 |

URL 不放密码或完整敏感数据；浏览器存储也不是正式数据库。共享某个 URL，不会把当前标签页的草稿一并交给别人。
...
## 9. JSON、JavaScript 和 Java 的数据不是一回事

### 9.1 JavaScript 对象是运行时的值
...
## 10. `sessionStorage`、Cookie 和 Session 的关系

### 10.1 `sessionStorage` 不是服务端 Session
...
### 11.6 区分无数据、无效 JSON 与错误结构

“没有保存过记录”“文本解析失败”“解析成功但不是数组”是三种不同状态。下面是独立控制台实验，只读取 `lesson18.applications`：

```js
function loadApplications() {
  const savedText = localStorage.getItem("lesson18.applications");
  if (savedText === null) {
    return [];
  }
...
const applications = safeLoadApplications();
if (applications === null) {
  console.log("读取失败，停止后续显示和写回");
} else {
  console.log("记录数：", applications.length);
}
```

`loadApplications()` 不接收参数：没有 key 返回空数组；正常解析数组则返回该数组；存储访问失败、文本损坏或结构不符时抛错。`safeLoadApplications()` 捕获这些错误，并按本例约定返回 `null` 表示失败。
...
| `[]` | 空数组，正常无记录 |
| `[{"id":"REQ-001"}]` | 含一条记录的数组 |
| `not-json` | 解析失败，记录错误并返回 `null` |
| `{}` 或 `null` | JSON 合法，但不是数组，返回失败标志 |

这个函数只验证最外层是数组；真实业务还应检查每条记录的字段与类型。不要把“解析成功”等同于“业务数据完全正确”。

### 11.7 key 缺失与 JSON 中的 null

`getItem()` 返回 JavaScript 的 `null`，表示没有这个 key；存储字符串 `"null"` 则表示 key 存在，只是 JSON 内容为 null。
...
第一行会先把参数转换成字符串 `"null"`，因此也能解析，不会仅因为参数为 `null` 就抛错。不能依靠解析是否报错判断 key 是否存在，应该像 11.6 节一样先判断 `getItem()` 的返回值。

### 11.8 数据损坏时不要自动覆盖

11.6 节用空数组表示正常无记录，用 `null` 表示读取失败，两者不能混同。如果发现损坏后立即把空数组写回，会丢失原数据和排查证据。

在独立练习页面使用以下方式验证：

1. 通过开发者工具为 `lesson18.applications` 依次设置 11.6 表格中的值，每次刷新运行读取脚本。
2. 对比控制台的记录数、错误消息和失败提示。
3. 错误状态下查看存储，确认原值没有被覆盖。
4. 把数据修正为合法数组再刷新，应恢复正常；实验结束只删除 `lesson18.applications`。

读取和写入都可能失败，实际页面应捕获异常、显示提示，并只在保存成功后更新“保存完成”的界面状态。
