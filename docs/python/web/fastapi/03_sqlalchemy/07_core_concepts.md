# 第7章 SQLAlchemy基础概念与常用对象

> 本章目标：先不进入员工项目代码，单独掌握 `SQLAlchemy` 这套数据库工具最核心的对象、安装方式、连接方式和模型定义方式。

本章建议分三个检查点完成：先准备数据库并用Engine执行`SELECT 1`；再建立Session并观察查询结果对象；最后定义ORM模型并说明Engine、Session、Model的边界。每完成一个检查点都运行一次`sqlalchemy_core_demo.py`。

本章完整Python实验统一放在`examples/sqlalchemy_core_demo.py`。标注为单行写法或对比写法的短代码块属于语法片段，用于说明当前对象，不需要重复加入文件。

## 学习前准备：建立独立数据库实验环境

第7～8章使用可丢弃MySQL练习数据库`training_db`，第9章单独使用`alembic_training_db`观察迁移，二者都不连接第10章开始使用的员工项目数据库。以下SQL以MySQL 8.x为例，使用数据库管理工具或MySQL客户端，以具备本地建库和账号管理权限的管理员身份执行一次：

```sql
CREATE DATABASE IF NOT EXISTS training_db  -- 创建独立练习数据库
    CHARACTER SET utf8mb4  -- 使用可保存中日文字符的编码
    COLLATE utf8mb4_unicode_ci;  -- 设置练习使用的排序规则

CREATE DATABASE IF NOT EXISTS alembic_training_db  -- 创建只用于第9章迁移的数据库
    CHARACTER SET utf8mb4  -- 使用可保存中日文字符的编码
    COLLATE utf8mb4_unicode_ci;  -- 设置练习使用的排序规则

CREATE USER IF NOT EXISTS 'fastapi_course'@'127.0.0.1'  -- 创建与连接地址匹配的本地练习账号
    IDENTIFIED BY '<local_password>';  -- 替换为只用于本机练习的密码

ALTER USER 'fastapi_course'@'127.0.0.1'  -- 确保练习账号使用本次设置的密码
    IDENTIFIED BY '<local_password>';  -- 与下面环境变量中的密码保持一致

GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, ALTER, INDEX, REFERENCES  -- 授予练习所需权限
    ON training_db.*  -- 权限只限定在training_db
    TO 'fastapi_course'@'127.0.0.1';  -- 指定与连接地址匹配的练习账号

GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, ALTER, INDEX, REFERENCES  -- 授予迁移练习权限
    ON alembic_training_db.*  -- 权限只限定在第9章迁移数据库
    TO 'fastapi_course'@'127.0.0.1';  -- 继续使用本地练习账号
```

不要使用生产数据库或日常工作数据库。`DROP`权限只用于第8章重建专用练习表；完成课程后可以由本地数据库管理员删除`training_db`和`fastapi_course`账号。

在当前PowerShell会话设置连接地址，密码建议使用只包含字母和数字的本地练习值；若包含`@`、`:`、`/`等字符，需要先进行URL编码：

```powershell
$env:SQLALCHEMY_TRAINING_URL = "mysql+pymysql://fastapi_course:<local_password>@127.0.0.1:3306/training_db?charset=utf8mb4"  # 设置第7～8章统一连接地址
$env:ALEMBIC_TRAINING_URL = "mysql+pymysql://fastapi_course:<local_password>@127.0.0.1:3306/alembic_training_db?charset=utf8mb4"  # 设置第9章迁移连接地址
```

关闭PowerShell后该环境变量会消失。重新上课时，进入`employee_api`、激活`.venv`并重新设置它。执行下面的命令确认变量已经存在，但不要把包含密码的完整输出截图或提交仓库：

```powershell
if ($env:SQLALCHEMY_TRAINING_URL -and $env:ALEMBIC_TRAINING_URL) { "training databases configured" }  # 只确认变量存在而不输出密码
```
...
DATABASE_URL = os.environ["SQLALCHEMY_TRAINING_URL"]  # 读取独立练习数据库连接地址
...
| 参数 | 作用 | 可接受的值 | 默认值或必填性 |
| --- | --- | --- | --- |
| `url` | 指定数据库连接地址 | SQLAlchemy URL字符串或`URL`对象 | 必填，可作为第一个位置参数 |
| `echo` | 控制是否输出SQL日志 | `True`、`False`或`"debug"` | 默认`False` |
| `pool_pre_ping` | 取连接前是否检查连接有效性 | `True`、`False` | 默认`False`；Web项目常设为`True` |

连接池还支持`pool_size`、`max_overflow`等容量参数，但它们依赖连接池类型、并发量和数据库限制。本章不修改这些参数；后续只有在测量到连接等待或容量问题时再单独调整。SQLAlchemy 2.x也不需要再设置旧版教程中的`future=True`。
...
```text
mysql+pymysql://fastapi_course:<local_password>@127.0.0.1:3306/training_db?charset=utf8mb4
```
...
| `mysql` | 目标数据库类型 |
| `pymysql` | 底层驱动 |
| `fastapi_course` | 第7～9章专用用户名 |
| `<local_password>` | 只保存在本地环境变量中的练习密码 |
| `127.0.0.1` | 主机地址 |
| `3306` | MySQL 端口 |
| `training_db` | 数据库名 |
...
DATABASE_URL = os.environ["SQLALCHEMY_TRAINING_URL"]  # 读取练习数据库连接地址
...
```python
text("SELECT 1")  # 把SQL字符串包装为可执行的SQLAlchemy文本语句
```
...
| 参数 | 作用 | 可接受的值 | 默认值或必填性 |
| --- | --- | --- | --- |
| `bind` | 绑定Session使用的数据库对象 | `Engine`、`Connection`或`None` | 默认`None`，项目中通常传入Engine |
| `autoflush` | 查询前是否自动把待处理变化flush到数据库 | `True`、`False` | 默认`True`；本例设为`False` |
| `autocommit` | 保留SQLAlchemy 2.x的非自动提交模式 | `False` | 默认`False`，不应设置为`True` |
...
| 参数 | 作用 | 可接受的值 | 默认值或必填性 |
| --- | --- | --- | --- |
| `autoflush` | 控制查询前是否自动flush待处理变化 | `True`、`False` | 默认`True`；本例设为`False` |
| `autocommit` | 保持SQLAlchemy 2.x显式事务模式 | `False` | 默认`False`，不能设置为`True` |
...
```python
__tablename__ = "users"  # 指定ORM模型映射到users表
```
...
| 参数 | 作用 | 可接受的值 | 默认值或必填性 |
| --- | --- | --- | --- |
| 第一个位置参数 | 指定数据库字段类型 | `Integer`、`String(100)`等SQLAlchemy类型 | 可省略，由`Mapped[...]`推断时不必传入 |
| `primary_key` | 声明主键 | `True`、`False` | 默认`False` |
| `autoincrement` | 控制整数主键自增 | `True`、`False`、`"auto"`等支持值 | 默认自动判断 |
| `unique` | 声明单字段唯一约束 | `True`、`False` | 默认`False` |
| `nullable` | 控制数据库是否允许`NULL` | `True`、`False`或由类型标注推断 | 默认根据类型和主键设置推断 |
| `default` | 设置Python侧插入默认值 | 固定值、可调用对象或`None` | 默认无Python侧默认值 |
...
文件：`examples/sqlalchemy_core_demo.py`  
操作：把本章标记为完整实验的代码保存在该文件后运行  
代码类型：独立实验程序

在`employee_api`项目根目录执行：

```powershell
python examples/sqlalchemy_core_demo.py
```

至少应看到`SELECT 1`的查询结果；启用`echo=True`时还会看到SQLAlchemy输出的SQL日志。连接失败时先检查MySQL服务、`SQLALCHEMY_TRAINING_URL`、练习账号权限和数据库名称，再进入第8章。
...
2. 根据`SQLALCHEMY_TRAINING_URL`说明MySQL连接字符串每一部分的作用，不把实际密码写入练习报告。
